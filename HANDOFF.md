...
- 新增 `scripts/validate-push-weekly.py`：只允许使用精确日期、未截断、且 26 个目标 `(workflow_name, task_name)` 都唯一匹配的响应。新增 `scripts/compare-push-weekly.py`：用于回归测试，逐项比较 26 个目标 × 5 个指标。`2026-08-12~18` 的浏览器重放已与人工抓取 **130/130 个值完全一致**，可作为后续自动重放的基线验证。
- `scripts/write-push-weekly.py` 默认仅 dry-run，`--apply` 才写飞书；无需再手动传 `--row-offset`，它会从每组六行模板中自动定位第一个 B 列及 D:H 均为空的周次行。
