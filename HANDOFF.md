...
### 把 `0812-0817` 覆盖成 `0812-0818`

**注意：这不是"额外补一期"，而就是周三那一轮的正常周期。** 按周三→周二规则，周三 2026-08-19 运行时刚结束的一期正是 08-12(周三) ~ 08-18(周二)。用户2026-08-18确认："需要补，但不是现在补，因为数据还没更新，明天运营的时候记得补"。

**2026-08-19 重试状态（未写入）**：已按 `2026-08-12 00:00:00` 至 `2026-08-18 23:59:59` 的绝对日期通过 API 重放抓到 78 行（无 `p_date` 维度、未触及 `limit=5000`、唯一键无重复，53 个目标行全可匹配）。但与飞书中现有 6 天版 `0812-0817` 比较，push、邮件、弹窗的曝光/点击/转化等累计值全部完全相同（均为 +0.0%），没有 08-18 的预期增量。用户选择等待 DataWind 数据刷新后再重试，因此本次未覆盖飞书；保留本小节和 `config/write-plan-0812-0818.json`，下次从重新抓取与量级对比开始，只有出现合理的 08-18 增量后才覆盖 117 / 112 / 83 行。
