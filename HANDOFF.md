...
**已知数据问题**：`0812-0817` 这一期是在**周二 2026-08-18** 跑的（比正常的周三提前一天），当天数据不完整所以只截到 08-17，导致这期只有6天。累计指标（曝光/点击/注册数）比7天口径低约14%，跟上下期做环比会有偏差。
**待处理**：建议在周三 2026-08-19 用 `--start 2026-08-12 --end 2026-08-18` 重跑一次，覆盖原来那53行，并把周期标签改成 `0812-0818`。名单和行号都不用变，只改日期和标签。

**下期**：`0819-0825`（08-19周三 ~ 08-25周二）。
